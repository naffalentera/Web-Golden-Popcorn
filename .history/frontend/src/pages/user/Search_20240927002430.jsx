...
const SearchPage = () => {
  ...
  // Apply sorting based on filtered data
useEffect(() => {
  let sorted = [...movies];
  ...
  setSortedMovies(sorted);  // Update sorted movies setelah filtering
}, [movies, sortBy]);  // Trigger efek ketika movies atau sortBy berubah
  ...
  const handleFilterChange = (filters) => {
    ...
    // Fetch data yang difilter dari backend
    fetch(`http://localhost:5000/api/movies?genre=${genre}&country=${country}&award=${award}&year=${year}`)
      .then(response => response.json())
      .then(data => {
          setMovies(data);  // Update state movies dengan hasil dari backend
      })
      .catch(error => {
